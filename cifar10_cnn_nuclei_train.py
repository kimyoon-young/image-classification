...
#Train a simple deep CNN on nuclei datasets.
...
def parse_single_example(record):
    ...
    image = tf.reshape(tensor = image, shape = [img_size,img_size,1])
...
model_name = 'keras_cifar10_nuclei_trained_model.h5'

# The data, split between train and test sets:


#load train tfrecord
train = tf.data.TFRecordDataset(filenames = './nuclei_train_matlab_00000-of-00001.tfrecords', compression_type = 'GZIP')
train = train.map(lambda record : parse_single_example(record))
train = train.batch(batch_size=1, num_threads = 8)
...
#load test tfrecord
test = tf.data.TFRecordDataset(filenames = './nuclei_validation_matlab_00000-of-00001.tfrecords', compression_type = 'GZIP')
test = test.map(lambda record : parse_single_example(record))
test = test.batch(batch_size = 1, num_threads = 8)
